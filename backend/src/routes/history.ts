...
export function historyRoutes(db: Database, jwtSecret: string): Router {
  ...
  router.post('/', (req: Request, res: Response) => {
    ...
    db.run(
      ...
      [id, userId, machine_id, app_id, version, action, status, JSON.stringify(details || {})]
    );

    // Update machine last_sync
    db.run("UPDATE machines SET last_sync = datetime('now') WHERE id = ?", [machine_id]);
    db.persist();

    res.status(201).json({
      event: { id, user_id: userId, machine_id, app_id, version, action, status },
    });
  });
