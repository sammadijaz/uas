...
function minimalPortableRecipe(): Record<string, unknown> {
  return {
    ...
    installer: {
      ...
      sha256: "d".repeat(64),
      portable: {
        copy_to: "${LOCALAPPDATA}\\portableapp",
        executable: "app.exe",
      },
    },
  };
...
describe("Semantic Validation", () => {
  it("should flag registry side effects without admin", () => {
    ...
    expect(result.valid).toBe(false);
    expect(
      result.errors.some((e) => e.rule === "semantic:registry-requires-admin"),
    ).toBe(true);
  });
  ...
  it("should flag hardcoded PATH entries", () => {
    ...
    expect(result.valid).toBe(false);
    expect(
      result.errors.some((e) => e.rule === "semantic:path-use-variables"),
    ).toBe(true);
  });
  ...
  it("should flag shell commands in installer", () => {
    ...
    expect(result.valid).toBe(false);
    expect(
      result.errors.some((e) => e.rule === "semantic:no-shell-commands"),
    ).toBe(true);
  });
  ...
  it("should flag mismatched installer type and options", () => {
    ...
    expect(
      result.errors.some(
        (e) =>
          e.rule === "semantic:type-options-match" ||
          e.rule.startsWith("schema:"),
      ),
    ).toBe(true);
  });
});
...
describe("Profiles", () => {
  ...
  it("should have apps in the profile", () => {
    const profile = catalog.loadProfile("frontend-dev");
    expect(profile).not.toBeNull();
    const apps = profile!.apps as Array<{
      id: string;
      version: string;
      optional: boolean;
    }>;
...
describe("Sample Recipe Validation", () => {
  ...
  it("should validate all sample recipes (schema only)", () => {
    ...
    for (const [id, result] of results) {
      ...
      expect(
        schemaErrors,
        `Schema errors in ${id}: ${JSON.stringify(schemaErrors)}`,
      ).toHaveLength(0);
    }
  });
