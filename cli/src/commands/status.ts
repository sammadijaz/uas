...
import { printInfo, printError, printTable, colors } from "../output";
...
function printAppDetails(app: InstalledApp): void {
  ...
  console.log(
    `  ${colors.bold("Installed:")}  ${formatDate(app.installed_at)}`,
  );
  console.log(
    `  ${colors.bold("Directory:")}  ${app.install_dir || colors.dim("(default)")}`,
  );
  console.log(
    `  ${colors.bold("Recipe:")}     ${colors.dim(app.recipe_hash.slice(0, 12) + "...")}`,
  );
