...
export function registerUninstallCommand(program: Command): void {
  program
    ...
    .action(async (app: string, opts: { dryRun: boolean; verbose: boolean }) => {
      ...
      if (opts.dryRun) {
        printDryRun(`Would uninstall ${colors.app(app)} v${colors.version(installed.version)}`);
      }
      ...
      try {
        ...
        if (result.final_state === 'COMPLETED') {
          if (opts.dryRun) {
            printDryRun(`Dry run complete — would uninstall ${colors.app(app)} v${colors.version(installed.version)}`);
          } else {
            printSuccess(
