...
export function registerProfileCommand(program: Command): void {
  ...
  profile
    ...
    .action(
      async (file: string, opts: { dryRun: boolean; verbose: boolean }) => {
        ...
        printInfo(
          `Applying profile: ${colors.bold(profileData.name)} (${profileData.apps.length} apps)`,
        );
        ...
        try {
          for (const appSpec of profileData.apps) {
            ...
            if (
              current &&
              (!appSpec.version ||
                appSpec.version === "latest" ||
                current.version === appSpec.version)
            ) {
              printInfo(
                `${colors.app(appSpec.id)} already installed (v${colors.version(current.version)}), skipping`,
              );
              skipped++;
              continue;
            }
            ...
            if (!recipe) {
              if (appSpec.optional) {
                printWarn(
                  `Optional app "${appSpec.id}" not found in catalog, skipping`,
                );
                skipped++;
              } else {
            ...
            if (result.final_state === "COMPLETED") {
              ...
              printError(
                `Failed to install ${recipe.id}: ${result.error?.message || result.final_state}`,
              );
              failed++;
            }
          }
