  ...
  program
    ...
    .action(async (app: string, opts: {
      ...
      if (!recipe) {
        ...
        printInfo(`Run ${colors.bold('uas search ' + app)} to find available apps.`);
        process.exit(1);
      }
      ...
      if (opts.dryRun) {
        printDryRun(`Would install ${colors.app(recipe.name)} v${colors.version(recipe.version)}`);
      }
      ...
      try {
        ...
        if (result.final_state === 'COMPLETED') {
          if (opts.dryRun) {
            printDryRun(`Dry run complete for ${colors.app(recipe.name)} v${colors.version(recipe.version)}`);
          } else {
            printSuccess(
          ...
          if (result.side_effects_applied.length > 0) {
            printInfo(`Applied ${result.side_effects_applied.length} side effect(s):`);
          ...
          if (result.error) {
            console.error(`  ${colors.dim('Category:')} ${result.error.category}`);
            console.error(`  ${colors.dim('Message:')}  ${result.error.message}`);
          }
          process.exit(1);
        }
      } catch (err) {
