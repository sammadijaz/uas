...
function createSampleRecipe(id: string, name: string, tags: string[] = []) {
  return {
    ...
    side_effects: {},
    metadata: { categories: ['tools'], tags, maintainer: 'test', updated: '2026-01-01' },
...
describe('Catalog Operations', () => {
  beforeEach(() => {
    ...
    const node = createSampleRecipe('node', 'Node.js', ['runtime', 'javascript']);
    const python = createSampleRecipe('python', 'Python', ['runtime', 'scripting']);
    const git = createSampleRecipe('git', 'Git', ['vcs', 'scm']);

    fs.writeFileSync(path.join(TEST_CATALOG, 'node.yaml'), stringifyYaml(node));
    fs.writeFileSync(path.join(TEST_CATALOG, 'python.yaml'), stringifyYaml(python));
  ...
  it('parses a valid recipe YAML', () => {
    const content = fs.readFileSync(path.join(TEST_CATALOG, 'node.yaml'), 'utf-8');
  ...
  it('lists YAML files in catalog directory', () => {
    const files = fs.readdirSync(TEST_CATALOG).filter((f: string) => f.endsWith('.yaml'));
  ...
  it('search filters by name', () => {
    ...
    const results = recipes.filter((r: any) => {
      const haystack = [r.id, r.name, r.description, ...r.metadata.tags].join(' ').toLowerCase();
  ...
  it('search filters by tag', () => {
    ...
    const query = 'runtime';
    const results = recipes.filter((r: any) => {
      const haystack = [r.id, r.name, ...r.metadata.tags].join(' ').toLowerCase();
      return haystack.includes(query.toLowerCase());
    });
